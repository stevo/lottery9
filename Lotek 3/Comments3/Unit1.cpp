...
Program dziala w oparciu o komponent TChecklistbox, w ktorym zaznaczamy wybrane numerki
Gdy klikamy w takim polu i zaznaczamy je, to jednoczesnie inkrementujemy licznik zaznaczonych elementow
(lub dekrementujemy odznaczajac).
Gdy zaznaczone jest szesc elementow, to wlaczamy przycisk losowania

Teraz podobnie jak w innych
Po odpaleniu losowaniwa wypelniamy tablice   unsigned int liczby_wybrane[6]; unsigned int liczby_wylosowane[6]; wylosowanymi liczbami (dzieki f-cji rand())
oraz tymi wyranymi (zczytanymi z pol CheckBox). Potaem sprawdzamy czy w obu tablicach powtarzaky sie te same liczby co oznacza wygrana. W tym wypadku zapisujemy liczby wytypowane i wylosowane w pliku poprze uzycie f-cji fwrite.
Tak czy inaczej inkrementujemy tablice z wylosowanymi liczbami (tzn. jeli element 51 ma zawartosc 5 tzn. ze liczba 51 wystapila w
losowaniach juz 5 razy)... Przed kolejnym losowaniem wypelniamy obie tablice zerami i odznaczamy wszystkie checkboxy
. Powodzenia!
...
struct losowanie
       {
        unsigned int liczby_wybrane[6]; //unsigned bo minusowych i tak nie bedziemy uwzgledniac
...
{
bool exist; //zmienna w ktorej zapamietamy czy wartos isntieje
exist=False;  // Zakladamy ze nie istnieje
 ...
 for (int i=0; i<6; i++)  //Dla kazdego elementu tej tablicy
   if (los.liczby_wylosowane[i]==numer)     //Jesli liczba jest rowna temu elementowi
    {
    exist=!exist;     // To zwaracamy true; (skoro exist bylo falso to przypisujemy mu NIE FALSE czyli TRUE)
...
{
Form1->Close();//zamykamy pierwsza formatke, czyli aplikacje
}
//---------------------------------------------------------------------------

void __fastcall TForm1::CheckListBox1ClickCheck(TObject *Sender)
{
int index = Form1->CheckListBox1->ItemIndex; //tu przechowuje,y index kliknietego elementu w lisicie
int count=0; zerujemy licznik zaznaczonych elementow

for (int i=0; i<Form1->CheckListBox1->Items->Count-1 ; i++) //Dla kazdego elementu tej listy (okienka z kratkami)
 {
  if (Form1->CheckListBox1->Checked[i]) count++; //sprawdzamy czy jest zaznaczone - jesli jest to inkrementujemy 						//licznik wystapien
 }

if (count>=6) Form1->BitBtn1->Enabled=True; else Form1->BitBtn1->Enabled=False; //jesli jest juz szesc liczb to                         //wlaczamy przycisk losowania, a jak nie to wylaczamy przycisk losowania

if (count>6) { // Jak zaznaczamy wiecej niz szesc to wywietlamy stosowny komunikat
...
int ind=0; //  tu przechowujemy licznik, potrzebny do wypalniania tablcy   los.liczby_wybrane                                                   

//Wypelnij tablice los.liczby_wybrane wybranymi 									//numerami
for (int i=0; i<Form1->CheckListBox1->Items->Count-1 ; i++)
 if (Form1->CheckListBox1->Checked[i]) los.liczby_wybrane[ind++]=i+1; //jesli element jest zazanaczony to wpisujemy go //do tablicy  i inkreentujemy wspomniany licznik
...
Frequency=fopen("freq.txt","r+");                   //Ponownie otwieramy plik
  ...
  for (int i=0; i<6; i++){    // wyswietl wszystie te liczby w StringGridzie
     ...
     Form1->StringGrid1->Cells[i][0]=los.liczby_wybrane[i];    // w pierwszym rzedzie te wybrane
     Form1->StringGrid1->Cells[i][1]=los.liczby_wylosowane[i]; // w drugim wylosowane
                         }
...
//sprawdzamy czy w wylosowanych sa takie liczby jakie wybralismy, jesli jest, to wpisujemy ja na kolejne...
    ...
    for (int i=0; i<6; i++)     //...i wyswietlamy zawartosc tej tablicy
      Form1->StringGrid1->Cells[i][2]=trafione[i];//... w trzecim wierszu string grida


if (jest_trafienie)    //Jesli trafilismy, to zapisujemy liczby wylosowane i wytypowane w pliku
 {
 FILE *win; //deklarujemy pomocnicza zmienna plikowa
  ...
  win=fopen("trafienia.txt","a+b");  //Odpalamy se plik w trybie binarnym i dopisywania na koniec
   fwrite(&los,sizeof(los),1,win);  //dodajemy do pliku wypelnion¹ strukture
     ...
     }   else   //Jesli losowanie zostalo odbyte, to trza wyczyscic wszystko...
     {
       ...
       for (int i=0; i<6; i++){    // Wyczysc wszystkie te liczby w StringGridzie (kazdy wiersz)
        Form1->StringGrid1->Cells[i][0]="";
        Form1->StringGrid1->Cells[i][1]="";
        Form1->StringGrid1->Cells[i][2]="";
                              }
       Form1->BitBtn1->Enabled=False; //deaktywuj przycisk losowania
