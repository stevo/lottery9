...
Program dziala w oparciu o pola Tedit do ktorych wpisujemy numerki...
Gdy opuszczamy takie pole np. przechodzac do drugiego to sprawdzane jest czy nie podalismy juz takiego numerku
czy czasami nie podalismy jakis liter albo co, lub tez czy podana liczba jest aby z zakresu od 1 - 49.
Po odpaleniu losowaniwa wypelniamy tablice int wylosowane[6]; i int wytypowane[6]; wylosowanymi liczbami (dzieki f-cji rand())
oraz tymi wyranymi (zczytanymi z pol Tedit). Potaem sprawdzamy czy w obu tablicach powtarzaky sie te same liczby co oznacza wygrana. W tym wypadku zapisujemy liczby wytypowane i wylosowane w pliku poprze uzycie f-cji fwrite.
Tak czy inaczej inkrementujemy talvie z wylosowanymi liczbami (tzn. jeli element 51 ma zawartosc 5 tzn. ze liczba 51 wystapila w
losowaniach juz 5 razy)... Przed kolejnym losowaniem wypelniamy obie tablice zerami i robimy porzadek z Teditami - czyli
sprawiamy aby nic nie bylo w nich wpisane i zeby miale normalne kolorki (no i zeby byly aktywne, czyli dalo sie tam cokolwiek wpisac)... Powodzenia!
...
for (int i=0; i<6; i++) // dla kazdego elementu tablicy z liczbami wylosowanymi
        {
...
if (!Trafienie) Form1->Memo1->Lines->Add("Niestety nic nie trafiles..."); else //wyswietlamy odpowiedni komounikat w //zaleznosci cz padlo trafienie
...
if ((PlikStatystyk=fopen("st.txt","r+"))==NULL)       //Otwieramy plik statystyk
 ShowMessage(AnsiString("Brak pliku statystyk")); else
   ...
   fread(&Statystyki,sizeof(Statystyki),1,PlikStatystyk);  //Zczytujemy jedn¹ tablice 
			//(fread (gdzie_mamy_zczytac,jak_duze_to_jest,ile_egzemplarzy_mamy_zczytac,z_jakiego_pliku)
...
if ((PlikWygranych=fopen("wyg.txt","a+b"))==NULL)  //Odpalamy se plik w trybie binarnym i dopisywania na koniec
 ShowMessage(AnsiString("Brak pliku z wygranymi!")); //Jak nie ma tego pliku to wyswietlamy komunikat (czyli fopen //zwrocilo NULL)
 else
   {
   fwrite(&tr,sizeof(tr),1,PlikWygranych);  //dodajemy do pliku wypelnion¹ strukture (podobna skladnia jak fread)
...
if (Trafienie) ZapiszWyniki(); //jesli bylo trafienie to wywolywujemy funkcje zapisujaca wyniki
UaktualnijStatystyki(); //wywoluemy funkcje zapisujaca statystyki
}
...
if ((atoi(Edit1->Text.c_str())>49) || (atoi(Edit1->Text.c_str())<0)) //Sprawdza czy podano liczbe z zakresu od 1 do 49
                        {
                          ...
                          Edit1->SetFocus();//ustawiamy kursor w polu ktore wlasnie opuszvczalismy
                        }
...
// TAK SAMO TO DZIALA DLA KAZDEGO KOLEJNEGO EDITA
}
//---------------------------------------------------------------------------
...
{
for (int i=0; i<6; i++)     //Wypelniamy obie tablice zerami
{
Wylosowane[i]=0;
Wytypowane[i]=0;
}
...
{
if ((PlikStatystyk=fopen("st.txt","r+"))==NULL)          // kolejne linijki Tak samo jak przy uaktualnianiu
 ShowMessage(AnsiString("Brak pliku statystyk")); else
   {
...
for (int i=1; i<50;i++)
{
 ...
 strcat(buf," = ");  // i znak = (strcat dokleja do stringa podanego jako pierwszy parametr, ten podany jako drugi parametr)
 ...
 Form1->Memo1->Lines->Add(buf); //i wyswietlamy to w memo
}
}
//---------------------------------------------------------------------------
void __fastcall TForm1::BitBtn3Click(TObject *Sender)
{
if ((PlikWygranych=fopen("wyg.txt","r+b"))==NULL)  //Odpalamy se plik w trybie binarnym i odczytywania (struktury musimy odczytywac //binarnie!!!)
 ShowMessage(AnsiString("Brak pliku z wygranymi!"));
 else
  {
       ...
       while (fread(&tr,sizeof(tr),1,PlikWygranych)) //dopoki udaje sie zczytac cala strukture to robimy to (fread zwraca NULL jak                                                     //mu sie nie uda zczytac calej struktury
       {
         Form1->Memo1->Lines->Add("---------------");
          char buf11[2]={'\0','\0'};            //Zerujemy tablice (bufory)... tak na wszelki wypadek (czyli kazda komorke                                                //wypelniamy wartoscia NULL (czyli inaczej '\0')
        ...
        strcat(buf12,buf11); //to juz bylo omawiane
         strcat(buf12,".");
